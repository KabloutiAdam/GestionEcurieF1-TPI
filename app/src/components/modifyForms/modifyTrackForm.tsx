...
export default function EditTrackForm({ isDisplayed, track }: Props) {
    ...
    const handleTrackSubmit = () => {
        if (name.length) {
            axios.post("/api/tracks/update", {
                name: name,
                trackLength: trackLength,
                oldTrackOrder: track?.trackOrder,
                newTrackOrder: order,
                country: nationalitySelected?.id,
                idTrack: track?.id
                ...
                <div className="w-full h-full grid grid-rows-[1fr_5fr_1fr] grid-cols-1">
                    ...
                    <div className="row-span-1 row-start-2 flex flex-col gap-2 justify-center">
                        ...
                        <div className="flex flex-col mb-6">
                            ...
                            <input
                                className={`ratingInput w-[20%] border-b-2 ml-5  focus:outline-none py-2 px-3 placeholder:italic ${trackLength.length == 0 ? "border-red-500" : "border-black"}`}
                                id="length"
                                type="number"
